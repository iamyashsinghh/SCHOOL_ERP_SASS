    ...
    <div class="min-h-screen flex">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-indigo-900 via-indigo-800 to-indigo-900 text-white transform transition-transform duration-300 ease-in-out lg:translate-x-0 -translate-x-full">
            ...
            <nav class="mt-6 px-3 space-y-1">
                <a href="{{ route('central.dashboard') }}" class="sidebar-link {{ request()->routeIs('central.dashboard') ? 'active' : 'text-indigo-200' }}">
                ...
                <a href="{{ route('central.ministries') }}" class="sidebar-link {{ request()->routeIs('central.ministries') ? 'active' : 'text-indigo-200' }}">
                ...
                @if(auth('central')->user()?->isPlatformOwner() || auth('central')->user()?->isMinistryAdmin())
                <a href="{{ route('central.provinces') }}" class="sidebar-link {{ request()->routeIs('central.provinces') ? 'active' : 'text-indigo-200' }}">
                ...
                <a href="{{ route('central.subdivisions') }}" class="sidebar-link {{ request()->routeIs('central.subdivisions') ? 'active' : 'text-indigo-200' }}">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    Sub-Divisions
                </a>

                <a href="{{ route('central.schools') }}" class="sidebar-link {{ request()->routeIs('central.schools') ? 'active' : 'text-indigo-200' }}">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                    Schools
                </a>

                <a href="{{ route('central.users') }}" class="sidebar-link {{ request()->routeIs('central.users') ? 'active' : 'text-indigo-200' }}">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    Users
                </a>

                @if(auth('central')->user()?->isPlatformOwner())
                <a href="{{ route('central.roles.index') }}" class="sidebar-link {{ request()->routeIs('central.roles.index') ? 'active' : 'text-indigo-200' }}">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                    Roles
                </a>
                @endif

                <a href="{{ route('central.audit') }}" class="sidebar-link {{ request()->routeIs('central.audit') ? 'active' : 'text-indigo-200' }}">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-indigo-700/50">
                <div class="flex items-center space-x-3">
                    <div class="w-9 h-9 bg-indigo-600 rounded-full flex items-center justify-center text-sm font-bold">
                        {{ strtoupper(substr(auth('central')->user()?->name ?? '', 0, 1)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium truncate">{{ auth('central')->user()?->name }}</p>
                        <p class="text-xs text-indigo-300 truncate">{{ auth('central')->user()?->role ?? 'Admin' }}</p>
                    </div>
                    <a href="{{ route('central.logout') }}" class="p-1.5 rounded-lg hover:bg-indigo-700 text-indigo-300 hover:text-white" title="Logout">
