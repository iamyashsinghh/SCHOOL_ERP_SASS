...
class QualificationImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        ...
        $logFile = $this->getLogFile('employee_qualification');

        [$errors, $rows] = $this->validate($rows);

        $this->checkForErrors('employee_qualification', $errors);
    ...
    private function validate(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            if ($course && strlen($course) > 100) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.course'), 'max', ['max' => 100]);
            }

            if ($session && strlen($session) > 100) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.session'), 'max', ['max' => 100]);
            }

            if ($institute && strlen($institute) > 100) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.institute'), 'max', ['max' => 100]);
            }

            if ($instituteAddress && strlen($instituteAddress) > 100) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.institute_address'), 'max', ['max' => 100]);
            }

            if ($affiliatedTo && strlen($affiliatedTo) > 100) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.affiliated_to'), 'max', ['max' => 100]);
            }

            if (! $result) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.result'), 'required');
            } elseif (! in_array(strtolower($result), QualificationResult::getKeys())) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.result'), 'invalid');
            }
            ...
            if ($startDate && ! CalHelper::validateDate($startDate)) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.start_date'), 'invalid');
            }
            ...
            if ($endDate && ! CalHelper::validateDate($endDate)) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.end_date'), 'invalid');
            }

            if ($startDate && $endDate && $startDate > $endDate) {
                $errors[] = $this->setError($rowNo, trans('employee.qualification.props.start_date'), 'date_before', ['date' => trans('employee.qualification.props.end_date')]);
            }

            if ($result == QualificationResult::PASS->value) {
                if (! $totalMarks) {
                    $errors[] = $this->setError($rowNo, trans('employee.qualification.props.total_marks'), 'required');
                } elseif (! is_numeric($totalMarks)) {
                    $errors[] = $this->setError($rowNo, trans('employee.qualification.props.total_marks'), 'numeric');
                }

                if (! $obtainedMarks) {
                    $errors[] = $this->setError($rowNo, trans('employee.qualification.props.obtained_marks'), 'required');
                } elseif (! is_numeric($obtainedMarks)) {
                    $errors[] = $this->setError($rowNo, trans('employee.qualification.props.obtained_marks'), 'numeric');
                }

                if ($totalMarks && $obtainedMarks && $totalMarks < $obtainedMarks) {
                    $errors[] = $this->setError($rowNo, trans('employee.qualification.props.total_marks'), 'min', ['min' => $obtainedMarks]);
                }
            }

            if ($result == QualificationResult::FAIL->value) {
                if (! $failedSubjects) {
                    $errors[] = $this->setError($rowNo, trans('employee.qualification.props.failed_subjects'), 'required');
                }
            }
