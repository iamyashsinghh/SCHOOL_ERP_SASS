...
class MarkRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        ...
        $validator->after(function ($validator) {
            ...
            $ranking = collect($newMarks)->groupBy('uuid')
                ->map(function ($items, $uuid) {
                    return [
                        'uuid' => $uuid,
                        'total_marks' => $items->sum(function ($item) {
                            return is_numeric($item['obtained_mark']) ? $item['obtained_mark'] : 0;
                        }),
                    ];
                });

            $ranking = $this->updateRanking($ranking->toArray(), 'total_marks');
