...
class BlockInchargeListService extends ListGenerator
{
    ...
    {
        $blocks = Str::toArray($request->query('blocks'));
        ...
        return Incharge::query()
            ->whereHasMorph(
                'model',
                [Block::class],
                function (Builder $query) use ($blocks) {
                    $query->when($blocks, function ($q, $blocks) {
                        $q->whereIn('uuid', $blocks);
                    });
                }
            )
    ...
    {
        return BlockInchargeResource::collection($this->filter($request)
