...
class GetPayableInstallment
{
    public function execute(Request $request, Student $student): Collection
    {
        ...
        $inputLateFee = $request->late_fee ?? 0;
        foreach ($studentFees as $studentFee) {
            if ($payableAmount <= 0) {
                break;
            }

            $installmentLateFee = $studentFee->calculateLateFeeAmount($date)->value;

            if (auth()->user()?->can('fee:customize-late-fee') && $request->has('late_fee')) {
                $adjustedLateFee = ($inputLateFee > $installmentLateFee) ? $installmentLateFee : $inputLateFee;
            } else {
                $adjustedLateFee = $installmentLateFee;
            }

            $totalLateFee += $adjustedLateFee;
            $inputLateFee -= $adjustedLateFee;

            $actualInstallment = $studentFee->getBalance($date)->value;

            $studentFeeBalance = $actualInstallment - $installmentLateFee + $adjustedLateFee;

            if ($studentFeeBalance > 0) {
                $payableStudentFeeIds[] = $studentFee->id;
            }

            $payableAmount -= ($payableAmount > $studentFeeBalance) ? $studentFeeBalance : $payableAmount;

            if ($installmentLateFee) {
                $studentFee->setMeta([
                    'custom_late_fee' => true,
                    'late_fee_amount' => $adjustedLateFee,
                    'original_late_fee_amount' => $installmentLateFee,
                ]);
                // only save if request has gateway (online payment) to avoid custom late fee related issue
        ...
        if ($hasCustomLateFee && $balance > $request->amount) {
            ...
            // logger('Amount paying: '.$request->amount);
        ...
        if ($request->late_fee && $lateFeeBalance > 0 && $lateFeeBalance < $request->late_fee) {
            // logger('Late Fee Balance: '.$lateFeeBalance);
            // logger('Requested Late Fee: '.$request->late_fee);
