...
class CalHelper
{
    ...
    public static function getDuration(?string $startDate = null, ?string $endDate = null, string $unit = 'year-month'): string
    {
        ...
        if ($unit == 'day') {
            return abs(Carbon::parse($endDate)->diffInDays(Carbon::parse($startDate))) + 1 .' '.trans('list.durations.days');
        }

        if ($unit == 'year-month-day') {
            $diff = Carbon::parse($endDate)->diff(Carbon::parse($startDate));

            $year = $diff->y > 0 ? $diff->y.' '.trans('list.durations.year') : '';
            $month = $diff->m > 0 ? $diff->m.' '.trans('list.durations.month') : '';
            $day = $diff->d > 0 ? $diff->d.' '.trans('list.durations.day') : '';
    ...
    public static function getRecentDates(string $date, int $totalDays = 7, string $direction = 'all'): array
    {
        $baseDate = Carbon::parse($date);

        $range = match ($direction) {
            'previous' => range(-$totalDays + 1, 0),
            'next' => range(0, $totalDays - 1),
            default => range(-intval(floor($totalDays / 2)), intval(floor($totalDays / 2))),
        };

        return collect($range)
            ->map(function (int $offset) use ($baseDate) {
                $currentDate = $baseDate->copy()->addDays($offset);
                $date = $currentDate->toDateString();

                return [
                    'date' => $date,
                    'label' => $currentDate->format('d M'),
                    'formatted' => \Cal::date($date)?->formatted,
                    'short_label' => $currentDate->format('d'),
                    'is_current' => $currentDate->isSameDay($baseDate),
                ];
            })
            ->toArray();
    }
