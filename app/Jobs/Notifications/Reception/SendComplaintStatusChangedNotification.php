...
class SendComplaintStatusChangedNotification implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $templateCode = 'complaint-status-changed';
        ...
        $inchargeUserIds = $incharges->map(function ($incharge) {
            return $incharge->employee?->contact?->user_id;
        })->filter()->toArray();

        $assignedTo = $incharges->map(function ($incharge) {
            return $incharge->employee?->contact?->name.' - '.$incharge->employee?->code_number;
        })->join(', ');
        ...
        $userIds = array_merge($userIds, $inchargeUserIds);
